...
use Web\MaestrasBundle\Entity\Cargo;
...
{    
    ...
    {        
        ...
        if ($metodo == 'POST') 
        {
            ...
            if ($id == 0 && $iud == 0) //NUEVO
            {
                ...
                $duplicado = $em->getRepository('MaestrasBundle:Cargo')->findOneByNombre($nm);
            ...
            {
                $em = $this->getDoctrine()->getManager();
                $eliminar = $em->getRepository('MaestrasBundle:Cargo')->Delete($id); 
    ...
    public function listaAction(Request $request)
    {
        ...
        if ($metodo == 'POST') 
        {
            ...
            $todo = $em->getRepository('MaestrasBundle:Cargo')->FindTodas($search, $start, $length);
            $recordsFiltered = 1;
            ...
            if ($todo)
            {
                ...
                $recordsTotal = $em->getRepository('MaestrasBundle:Cargo')->CountTodos();                
    ...
    {
        $em = $this->getDoctrine()->getManager();
        $modificar = $em->getRepository('MaestrasBundle:Cargo')->Update($id,$nombre,$descripcion,$estado); 
